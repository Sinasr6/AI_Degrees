...
def shortest_path(source, target):
    ...
    """
    # initialzing variabales
    
    solution = []
    start = Node(state = source, parent = None, action = None)
    frontier = QueueFrontier()
    frontier.add(start)
    explored_set = [] #the source code uses set. must examine why later
    
    # setting the repeat for solve
    ...
    while (True):
        ...
        if r_node.state == target:
            while r_node.parent is not None:
                solution.append((r_node.action, r_node.state))
                r_node = r_node.parent
            solution.reverse()
            return solution
        
        # add name_id to explored set
        
        explored_set.append(r_node.state)
        
        # expanding the r_node
        ...
        for (movie_id, person_id) in neighbors_for_person(r_node.state):
            if not frontier.contains_state(person_id) and person_id not in explored_set:
                child = Node(state = person_id, parent = r_node, action = movie_id)
                frontier.add(child)
